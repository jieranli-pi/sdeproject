...
for i in range(len(p_values)):
    ...
    for sim in range(num_simulations):
        for j, dt_factor in enumerate(dt_factors):
            dt = dt_ref / dt_factor
            N = int(T / dt)

            # Perform Euler scheme simulation strong
            S_euler, _, _ = euler_scheme(mu, S0, sigma0, xi0, p_values[i], alpha_values[i], T, N)
            #euler_error = np.abs(S_euler[::dt_factor] - S_ref).max()
            #euler_error = np.abs(np.sum((S_euler[::dt_factor]**2 - S_ref**2))/len(S_euler))
            euler_error = np.abs(np.sum((S_euler[::dt_factor] - S_ref))/len(S_euler))
            ...
            #milstein_error = np.abs(S_milstein[::dt_factor] - S_ref).max()
            #milstein_error = np.abs(np.sum((S_milstein[::dt_factor]**2 - S_ref**2))/len(S_milstein))
            milstein_error = np.abs(np.sum((S_milstein[::dt_factor] - S_ref))/len(S_milstein))
...
# Arrays to store convergence rates and orders
euler_convergence_rates = []
milstein_convergence_rates = []
euler_convergence_orders = []
milstein_convergence_orders = []
...
for i in range(len(p_values)):
    ...
    # Fit a line to the log-log plot for convergence rate calculation
    popt_euler, _ = curve_fit(fit_line, np.log(dt_factors), np.log(euler_dt_errors))
    popt_milstein, _ = curve_fit(fit_line, np.log(dt_factors), np.log(milstein_dt_errors))

    # Store fitting parameters
    euler_fit_params.append(popt_euler)
    milstein_fit_params.append(popt_milstein)

    # Calculate convergence rates and orders
    euler_convergence_rates.append(popt_euler[0])
    milstein_convergence_rates.append(popt_milstein[0])
    euler_convergence_orders.append(-popt_euler[0])
    milstein_convergence_orders.append(-popt_milstein[0])
...
for i in range(len(p_values)):
    ...
    # Plot fitting lines
    plt.plot(dt_factors, np.exp(fit_line(np.log(dt_factors), *euler_fit_params[i])), '--', color='C0', alpha=0.5)
    plt.plot(dt_factors, np.exp(fit_line(np.log(dt_factors), *milstein_fit_params[i])), '--', color='C1', alpha=0.5)

# Print convergence rates and orders
for i in range(len(p_values)):
    print(f'Euler (p={p_values[i]}, alpha={alpha_values[i]}): Convergence Rate = {euler_convergence_rates[i]}, Convergence Order = {euler_convergence_orders[i]}')
    print(f'Milstein (p={p_values[i]}, alpha={alpha_values[i]}): Convergence Rate = {milstein_convergence_rates[i]}, Convergence Order = {milstein_convergence_orders[i]}')
